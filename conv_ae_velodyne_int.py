...
def create_network(x_input, number_fc, fc_widths):
    ...
    x = tf.reshape(x_input[:,:,:,1], [tf.shape(x_input)[0], 900, 16, 1], name='reshape_image1')
...
for i in range(2,fc_array.shape[0]):
    ...
    path_model = "../data/20180201/models/conv_ae_velodyne_int_3_" + str(fc_size_array[i,0]) + "_" + str(fc_size_array[i,1]) + "_" + str(fc_size_array[i,2]) + "_" + str(number_of_fc) + "_" + str(number_of_conv) + ".ckpt"
